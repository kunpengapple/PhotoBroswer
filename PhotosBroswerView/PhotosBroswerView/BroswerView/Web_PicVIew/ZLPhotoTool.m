    ...
    [smartAlbums enumerateObjectsUsingBlock:^(PHAssetCollection * _Nonnull collection, NSUInteger idx, BOOL *stop) {
        ...
        if(collection.assetCollectionSubtype != 202 && collection.assetCollectionSubtype < 212){
            ...
            if (assets.count > 0) {
                ...
                if (assets.count>1) {
                    ablum.secondImageAsset = [assets objectAtIndex:1];
                }
                if (assets.count>2) {
                    ablum.thridImageAsset = [assets objectAtIndex:2];
                }
...
- (NSArray *)updateAssetCollections:(PHFetchResult *)fetchResult
{
    // Filter albums
...
- (NSArray<ZLPhotoAblumList *> *)getPhotoAblumListByResultArray:(NSArray *)resultArray {
    ...
    //获取所有智能相册
    PHFetchResult *smartAlbums = resultArray[0];
    NSArray *assetCollection = [self updateAssetCollections:smartAlbums];
    for (PHAssetCollection *collection in assetCollection) {
        //过滤掉视频和最近删除
        if(collection.assetCollectionSubtype != 202 && collection.assetCollectionSubtype < 212){
            NSArray<PHAsset *> *assets = [self getAssetsInAssetCollection:collection ascending:NO];
            if (assets.count > 0) {
    ...
    //获取用户创建的相册
    PHFetchResult *userAlbums = resultArray[1];
    [userAlbums enumerateObjectsUsingBlock:^(PHAssetCollection * _Nonnull collection, NSUInteger idx, BOOL * _Nonnull stop) {
        NSArray<PHAsset *> *assets = [self getAssetsInAssetCollection:collection ascending:NO];
        if (assets.count > 0) {
            ZLPhotoAblumList *ablum = [[ZLPhotoAblumList alloc] init];
...
- (void)requestImageForAsset:(PHAsset *)asset isSynchronous:(BOOL)synchronous size:(CGSize)size resizeMode:(PHImageRequestOptionsResizeMode)resizeMode completion:(void (^)(UIImage *image))completion
{
    ...
    if (requestID >= 1 ) {//&& size.width/asset.pixelWidth == [UIScreen mainScreen].scale原判断,可能有用
...
//        option.resizeMode = resizeMode;//控制照片尺寸
    ...
    option.networkAccessAllowed = YES;//从iCloud下载图片
...
- (void)getPhotosBytesWithArray:(NSArray *)photos completion:(void (^)(NSString *photosBytes))completion
{
    ...
    for (int i = 0; i < photos.count; i++) {
        ...
        [[PHCachingImageManager defaultManager] requestImageDataForAsset:model options:nil resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
...
- (NSString *)shouldAddAssetToArray:(PHAsset *)asset {
    CGFloat maxLength = (asset.pixelWidth > asset.pixelHeight)? asset.pixelWidth : asset.pixelHeight;
//    float minLength = (asset.pixelWidth < asset.pixelHeight)? asset.pixelWidth : asset.pixelHeight;
//    NSLog(@"%f///",maxLength);
    if (maxLength < 1100) {
        return @"图片过小,无法展示,请重新选择";
